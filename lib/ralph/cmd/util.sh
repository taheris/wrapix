...
# Detect profile from spec content
# Usage: detect_profile "$spec_content"
# Returns: profile name (rust, python, or base)
#
# Profile detection heuristics (from ralph-workflow spec):
# | Signal | Profile |
# |--------|---------|
...
detect_profile() {
  ...
  # Check for Rust signals
  # - .rs file extensions in Affected Files table or elsewhere
  # - Cargo.toml references
  # - Keywords: Rust, cargo, rustc, rustfmt
  if echo "$content" | grep -qiE '\.rs[^a-z]|\.rs$|Cargo\.toml|[^a-z]Rust[^a-z]|[^a-z]cargo[^a-z]|rustc|rustfmt'; then
    has_rust=true
    debug "Profile detection: found Rust signals"
  fi

  # Check for Python signals
  # - .py file extensions
  # - pyproject.toml, requirements.txt, setup.py references
  # - Keywords: Python, pytest, pip, venv
  if echo "$content" | grep -qiE '\.py[^a-z]|\.py$|pyproject\.toml|requirements\.txt|setup\.py|[^a-z]Python[^a-z]|pytest|[^a-z]pip[^a-z]|[^a-z]venv[^a-z]'; then
    has_python=true
    debug "Profile detection: found Python signals"
  fi

  # Determine profile based on signals
  # If mixed or unclear, default to base
  if [ "$has_rust" = true ] && [ "$has_python" = false ]; then
    echo "rust"
  elif [ "$has_python" = true ] && [ "$has_rust" = false ]; then
    echo "python"
  else
    # Mixed, unclear, or only .nix files -> base
    echo "base"
  fi
}

# Detect profile for a specific task description
# Usage: detect_task_profile "$task_description" "$spec_content" "$default_profile"
# Returns: profile name for this specific task
#
# For multi-profile specs, tasks may need different profiles.
# This function checks the task description for language-specific hints
# and falls back to the default profile if unclear.
# shellcheck disable=SC2034  # spec_content reserved for future context-aware detection
detect_task_profile() {
  local task_desc="$1"
  local spec_content="$2"  # Reserved for future enhanced detection
  local default_profile="${3:-base}"

  # Check if task description mentions specific language/toolchain
  local task_profile
  task_profile=$(detect_profile "$task_desc")

  # If task-specific detection found something, use it
  # Otherwise use the default (spec-level) profile
  if [ "$task_profile" != "base" ]; then
    echo "$task_profile"
  else
