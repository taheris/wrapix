...
`pkgs.playwright-mcp` from nixpkgs provides the `mcp-server-playwright` binary. It already wraps `PLAYWRIGHT_BROWSERS_PATH` to point at `playwright-driver.browsers` and sets `PLAYWRIGHT_MCP_BROWSER=chromium`. However, it has a known bug ([nixpkgs#443704](https://github.com/NixOS/nixpkgs/issues/443704)) where Playwright tries to write a browser profile into the read-only Nix store.

The recommended approach:
1. Use `pkgs.playwright-mcp` as the server binary (offline, deterministic)
2. Set `PLAYWRIGHT_BROWSERS_PATH` to a writable location at runtime (e.g., `/tmp/playwright-browsers`) to work around the store write bug
3. Pass `--executable-path` explicitly to the Nix store chromium binary, derived from `playwright-driver`'s `browsers.json`:

```nix
let
  browsers = builtins.fromJSON (builtins.readFile "${pkgs.playwright-driver}/browsers.json");
  chromiumEntry = builtins.head (builtins.filter (b: b.name == "chromium") browsers.browsers);
  chromiumPath = "${pkgs.playwright-driver.browsers}/chromium-${chromiumEntry.revision}/chrome-linux64/chrome";
in
  # use chromiumPath as --executable-path
```

This separates the binary lookup (explicit path, read-only Nix store) from the profile/state directory (writable tmpdir).

### tmux-debug Rename

The `tmux-debug` MCP server should be renamed to `tmux` as part of this work. Changes required:

- `lib/mcp/default.nix` — Registry key: `tmux-debug` to `tmux`
- `lib/mcp/tmux/default.nix` — `name = "tmux-debug"` to `name = "tmux"`
- `lib/mcp/tmux/tmux-debug-mcp/` — Consider renaming crate to `tmux-mcp` (binary `tmux-mcp`)
- Consumer flakes — `mcp.tmux-debug = {}` to `mcp.tmux = {}`
- Environment variables — `TMUX_DEBUG_AUDIT` to `TMUX_AUDIT` (and `TMUX_DEBUG_AUDIT_FULL` to `TMUX_AUDIT_FULL`)
- Tmux session name — `debug-{pid}` to `tmux-{pid}`
- Update `specs/tmux-mcp.md` to reflect the rename

### Invocation Model

Both `tmux` and `playwright` use the same invocation model: tools are registered in the main Claude session when enabled via `mcp = { ... }`. The tmux-debug spec has already been updated to remove the prescribed subagent isolation.
