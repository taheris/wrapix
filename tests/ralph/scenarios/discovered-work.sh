...
phase_ready() {
  ...
  local main_task_id
  main_task_id=$(bd create --title="Main Task - discovers additional work" --type=task --labels="rl-$label" --silent 2>/dev/null)
...
phase_step() {
  ...
  if [ "$discover_type" = "sequential" ]; then
    ...
    discovered_json=$(bd create --title="Discovered Sequential Work - prerequisite" --type=task --labels="rl-$label" --json 2>/dev/null)
    local discovered_id
    ...
    # Create the discovered task
    local discovered_json
    discovered_json=$(bd create --title="Discovered Parallel Work - independent" --type=task --labels="rl-$label" --json 2>/dev/null)
    local discovered_id
    discovered_id=$(echo "$discovered_json" | jq -r '.id')
    echo "Created discovered task: $discovered_id"
    ...
    # Sequential task
    local seq_json
    seq_json=$(bd create --title="Discovered Sequential Work - must do first" --type=task --labels="rl-$label" --json 2>/dev/null)
    local seq_id
    ...
    # Parallel task
    local par_json
    par_json=$(bd create --title="Discovered Parallel Work - can do later" --type=task --labels="rl-$label" --json 2>/dev/null)
    local par_id
